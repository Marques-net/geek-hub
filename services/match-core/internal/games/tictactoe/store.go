...
func (s *Store) ListRooms(ctx context.Context) ([]*RoomState, error) {
	pattern := fmt.Sprintf("%s:%s:room:*", s.config.RedisKeyPrefix, GameTypeTicTacToe)
	cursor := uint64(0)
...
func (s *Store) roomKey(roomCode string) string {
	return fmt.Sprintf("%s:%s:room:%s", s.config.RedisKeyPrefix, GameTypeTicTacToe, roomCode)
}
