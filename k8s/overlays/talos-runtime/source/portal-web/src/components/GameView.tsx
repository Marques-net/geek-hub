...
import { ClientSession, Color, RoomSnapshot, SnapshotEnvelope } from "../types";
  ...
  switch (snapshot.status) {
    ...
    case "active":
      if (
        ...
        return "Partida ativa, relógio aguardando o primeiro lance";
      }
    ...
    case "timeout":
      return snapshot.winner === "w" ? "Vitória das brancas no relógio" : "Vitória das pretas no relógio";
    default:
...
const describeClockMode = (snapshot: RoomSnapshot): string => {
  ...
  if (snapshot.status === "active" && snapshot.moveHistory.length === 0 && snapshot.clocks.turnStartedAt === null) {
    return `Relógio de ${baseClock} por jogador. Ele começa no primeiro lance das brancas.`;
  }
      ...
      <div className="side-column">
        <div className="panel">
          ...
          <p className="lead">
            {isBotGame
              ? isPlayer
                ? "Modo treino contra a máquina easy. Você joga com as brancas e o backend responde com lances simples para iniciantes."
                : "Sala de treino contra a máquina easy. Você está assistindo como espectador."
              : isPlayer
                ? `Você joga com ${session.color === "w" ? "as brancas" : "as pretas"}.`
